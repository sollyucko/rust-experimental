...
generate_assert_eq_vec_and_prim! { assert_eq_vec_and_slice_mut      <B>(&mut [B]) }
generate_assert_eq_vec_and_prim! { assert_eq_vec_and_array_3        <B>([B; 3]) }
generate_assert_eq_vec_and_prim! { assert_eq_vec_and_ref_array_3    <B>(&[B; 3]) }
generate_assert_eq_vec_and_prim! { assert_eq_vec_and_ref_mut_array_3<B>(&mut [B; 3]) }

#[test]
fn partialeq_vec_and_prim() {
    assert_eq_vec_and_slice(vec![1, 2, 3], &[1, 2, 3]);
    assert_eq_vec_and_slice_mut(vec![1, 2, 3], &mut [1, 2, 3]);
    assert_eq_vec_and_array_3(vec![1, 2, 3], [1, 2, 3]);
    assert_eq_vec_and_ref_array_3(vec![1, 2, 3], &[1, 2, 3]);
    assert_eq_vec_and_ref_mut_array_3(vec![1, 2, 3], &mut [1, 2, 3]);
}
...
fn partialeq_vec_full() {
    ...
    let slice2: &[_] = &[1, 2];
    let slice3: &[_] = &[1, 2, 3];
    let slicemut2: &mut [_] = &mut [1, 2];
    let slicemut3: &mut [_] = &mut [1, 2, 3];
    let array2: [_; 2] = [1, 2];
    let array3: [_; 3] = [1, 2, 3];
    let arrayref2: &[_; 2] = &[1, 2];
    let arrayref3: &[_; 3] = &[1, 2, 3];
    let arrayrefmut2: &mut [_; 2] = &mut [1, 2];
    let arrayrefmut3: &mut [_; 3] = &mut [1, 2, 3];
    ...
    assert_partial_eq_valid!(vec2,vec3; arrayrefmut2,arrayrefmut3);
}
